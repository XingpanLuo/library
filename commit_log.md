...
1. database还书更改了书籍状态
2. master中书籍状态由数字改为中文
3. 更新了借书的feature（一个人借同一本书只能借阅一次）

## 提交26 by lxp

1. 管理员界面标题栏添加头像
2. 标题栏头像位置修改
3. 修复个人信息修改后，标题栏头像未修改的bug
4. 增加功能：不管是读者还是管理员，登录是检查所有没有归还的借阅历史，查看是否违期，如果违期，加到违期表
5. 修改借书逻辑，借书时还书日期为NULL
...
1. 根据借阅表修改而简化读者逻辑
2. 增加限制：每个人同时最多借阅3本，超过此数量不可借阅（但是可以预约），见 reader.py 204行
